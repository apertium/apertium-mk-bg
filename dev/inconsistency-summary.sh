...
for i in $POS; do
	if [ "$i" = "det" ]; then
		...
		TOTAL=`cat $INC | grep "<$i>" | grep -v 'art<det>' | grep -v REGEX | wc -l`; 
		AT=`cat $INC | grep "<$i>" | grep '@' | grep -v 'art<det>'  | grep -v REGEX | wc -l`;
		HASH=`cat $INC | grep "<$i>" | grep '>  *#' | grep -v 'art<det>' | grep -v REGEX |  wc -l`;
	else
